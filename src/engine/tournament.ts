...
function computeStandings(players: Player[], matches: SwissMatch[]): StandingRow[] {
  ...
  matches.forEach((m) => {
    ...
    const s1 = stats[m.p1Id], s2 = stats[p2Id];
    if (!s1 || !s2 || !m.result) return;
    s1.matchesPlayed += 1; s2.matchesPlayed += 1;
    s1.opponents.push(p2Id); s2.opponents.push(m.p1Id);
...
function generateSwissPairings(players: Player[], matches: SwissMatch[], roundNumber: number): SwissPairingResult {
...
function createSingleEliminationBracket(players: Player[]): SingleEliminationBracket {
  const size = nextPowerOfTwo(players.length);
  const seedOrder = generateSeedOrder(size);
  const slots: (Player | null)[] = seedOrder.map((seedNum) => players[seedNum - 1] || null);
...
function reportSingleEliminationResult(bracket: SingleEliminationBracket, matchId: string, winnerId: string): SingleEliminationBracket {
...
// ===================== Double elimination =====================
function createDoubleEliminationBracket(players: Player[]): DoubleEliminationBracket {
  const size = nextPowerOfTwo(players.length);
  ...
  const grandFinal: GrandFinalMatch = { id: "GF", p1Id: null, p2Id: null, winnerId: null };
  const grandFinalReset: GrandFinalMatch & { active: boolean } = { id: "GF2", p1Id: null, p2Id: null, winnerId: null, active: false };

  const bracket: DoubleEliminationBracket = { size, R, wbRounds, lbRounds, grandFinal, grandFinalReset };
...
function reportDoubleEliminationResult(bracket: DoubleEliminationBracket, matchId: string, winnerId: string): DoubleEliminationBracket {
